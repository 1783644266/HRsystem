  ...
  <div class="user-info">
    ...
    <el-form label-width="220px">
      <el-row type="flex" justify="end">
          <el-button type="text" style="margin-right: 40px" @click="$router.push(`/employees/print/${userId}?type=personal`)">
  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.headPortrait.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    }, // 获取员工个人信息
    async savePersonal() {
      const fileList = this.$refs.photo.fileList
      if (fileList.some(e => !e.upload)) {
    ...
    async saveUser() {
      const fileList = this.$refs.headPortrait.fileList
      if (fileList.some(e => !e.upload)) {
        return this.$message.warning('图片正在上传中，请稍候！')
      }
